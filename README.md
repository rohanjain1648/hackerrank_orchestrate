...
![Gemini](https://img.shields.io/badge/Powered%20By-Gemini%201.5-purple?style=for-the-badge&logo=google)
...
-   **Stage 2: Deterministic Routing**: Hardcoded rules for sensitive topics (Billing, Fraud, PII).
-   **Stage 3: Optimized RAG**: An in-memory TF-IDF semantic search engine for instant documentation retrieval.
-   **Stage 4: LLM Reasoning**: Gemini 1.5 Pro analyzes the grounded context to generate precise, empathetic responses.
...
2.  **Indexing**: A NumPy-optimized TF-IDF matrix is built in-memory.
...
-   **Search Engine**: NumPy-based TF-IDF Vectorization
-   **LLM**: Google Gemini 1.5 Pro (Primary) / OpenAI GPT-4o (Fallback)
-   **Safety**: Custom Regex Pipeline

---

## 🤖 AI Deep Dive
### Grounded Reasoning (RAG)
Unlike standard bots, our agent uses a **strictly grounded prompt**. It is instructed to *never* use internal knowledge. The context window is populated with the top 10 most relevant documentation chunks, ensuring 100% policy compliance.
...
## ⚖️ Trade-offs
-   **TF-IDF vs. Dense Embeddings**: We chose TF-IDF for its speed and zero dependency on remote embedding APIs, which is critical for 24-hour hackathon reliability.
...
## 🏆 Why This Will Win
The **Orchestrate Agent** isn't just an LLM wrapper; it's a **resilient support system**. By combining the speed of classical NLP (TF-IDF) with the reasoning of state-of-the-art LLMs (Gemini), we've built a tool that is faster, safer, and more reliable than generic AI solutions.
...
## 📚 Lessons Learned
-   **Rate Limiting is Crucial**: Handling 429 errors from LLM providers required a robust retry-with-exponential-backoff strategy.
